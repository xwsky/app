...
public class ImageUtil {
	 ...
	 * @return
	 */
	public static String generatePath(String inPath, Integer width, Integer height){
		StringBuilder sb = new StringBuilder();
		if(width == null && height != null){
			sb.append(inPath.substring(0, inPath.lastIndexOf("."))).append("_").append(height).append("h").append(inPath.substring(inPath.lastIndexOf(".")));			
		}else if(width != null && height == null){
			sb.append(inPath.substring(0, inPath.lastIndexOf("."))).append("_").append(width).append("w").append(inPath.substring(inPath.lastIndexOf(".")));			
		}else{
			sb.append(inPath.substring(0, inPath.lastIndexOf("."))).append("_").append(width).append("w").append(height).append("h").append(inPath.substring(inPath.lastIndexOf(".")));		
		}
		return sb.toString();
	}
	...
	public static String[] scalByWidth(String inPath, int[] widths,
		...
		try {
			...
			for (int i = 0; i < widths.length; i++) {
				String path = null;
				if (outPath == null) {
					path = generatePath(inPath, widths[i], null);
					outPaths[i] = path;
				} else {
					path = outPath[i];
				}
	...
	public static String[] scalByHeight(String inPath, int[] heights,
		...
		try {
			...
			for (int i = 0; i < heights.length; i++) {
				String path = null;
				if (outPath == null) {
					path = generatePath(inPath, null, heights[i]);
					outPaths[i] = path;
				} else {
					path = outPath[i];
				}
		...
		try {
			...
			for (int i = 0; i < (heights != null ? heights.length
				...
				if (widths == null && heights != null) {
					...
					if (outPath == null) {
						path = generatePath(inPath, widths[i], heights[i]);
						outPaths[i] = path;
					} else {
						path = outPath[i];
					}
     ...
     * @param is
     */
    public static void outImageResponse(HttpServletResponse response, BufferedImage image) throws Exception{
    ...
    public static void main(String[] args) {
    	int[] a = {50, 60, 70};
    	int[] b = {50, 60, 70};
    	ImageUtil.scalByWidthAndHeight("E:/18.jpg", a, b, null);
	}
