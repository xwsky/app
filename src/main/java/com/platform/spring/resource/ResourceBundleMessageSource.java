...
package com.platform.spring.resource;
...
//				System.out.println(basename.trim());
...
//		}
//		
//		System.out.println(this.basenames[0] + "--" + this.basenames[1]);
		...
		if (basenames != null && basenames.length > 0) {
			 ...
			 try{
				 for(String basename : basenames){
					 String allPath = path + basename;
					 File directoryfile = new File(allPath);
					 if(!directoryfile.isDirectory()){
						continue;
					 }
					 for(File file : directoryfile.listFiles()){
						 String filePath = file.getPath();
						 if(file.getPath().indexOf("properties") != -1){
							 String fileFullName = FileUtil.getFileFullNameByPath(filePath);
							 String fileName = null;
							 int index = fileFullName.indexOf("_");
							 if(index != -1){
								 fileName = fileFullName.substring(0, index);
								 set.add(basename + "/" + fileName);
							 }		
						 }
					 }
				 }
				 ...
				 if(set.size() > 0){
					 ...
					 this.basenames = new String[set.size()];
					 
					 int i = 0;
					 
					 for(String fileName : set){
						 logger.info("Loading i18n file：" + fileName);
						 this.basenames[i] = fileName;
						 i++;
					 }
