...
public class FileUtil {
	...
	public static String getFileFullNameByPath(String path){		
		...
		int b = path.lastIndexOf(File.separator) + 1; 
		String fileFullName = null;
		if(a > b)fileFullName=path.substring(a);
		else fileFullName=path.substring(b); 
		return fileFullName;
	}	
	...
	public static String getFileNameByPath(String path){	
		String fullName = getFileFullNameByPath(path);
	...
	/**
	 * 删除文件
	 * @param delpath		文件路径或文件夹路径
	 * @param filter		如果是文件夹可根据文件名过滤要删除的文件
	 * @return
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	public static boolean deleteFiles(String delpath, String filter)
			...
			for (int i = 0; i < filelist.length; i++) {
				...
				if (!delfile.isDirectory()) {	
					...
					if(TextUtil.isNotEmpty(filter)){
						execute = (delfile.getName().indexOf(filter) != -1);													
					}
					if(execute){
						delfile.delete();
					}
				} else if (delfile.isDirectory()) {
					deleteFiles(delpath + File.separatorChar + filelist[i], filter);
				}
			}			
			// 删除文件夹
			// file.delete();
